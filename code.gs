...
function handleChunkFailure_(file, state, result, index) {
  if (result.needsSplit && state.chunks[index].length > CONFIG.MIN_SPLIT_CHARS) {
    const text = state.chunks[index]; const midpoint = Math.floor(text.length / 2); let splitIndex = text.lastIndexOf('\n', midpoint);
    if (splitIndex < text.length * 0.25 || splitIndex > text.length * 0.75) splitIndex = midpoint;
    const partA = text.substring(0, splitIndex).trim(); const partB = text.substring(splitIndex).trim();
    if (partA && partB) { state.chunks.splice(index, 1, partA, partB); state.retryCount = 0; state.nextAttemptAt = Date.now() + CONFIG.GROQ_BACKOFF_MS; saveState_(file, state); return 'WAIT'; }
  }
...
function handleModelFailure_(file, state, result, stageName) {
  ...
  if (result && result.incomplete) {
    ...
    if (retries < 2) { state.finalizeRetryCount = retries + 1; state.stage = 'FINALIZE'; state.nextAttemptAt = Date.now() + 10000; state.deliveryError = `Incomplete output during ${stageName}; retry ${state.finalizeRetryCount}/2.`; saveState_(file, state); console.warn(`Job ${state.jobId}: incomplete output; retrying ${stageName}.`); return 'WAIT'; }
  }
  if (result && result.rateLimited && Number(state.retryCount || 0) < CONFIG.MAX_RETRIES) { state.retryCount = Number(state.retryCount || 0) + 1; state.nextAttemptAt = Date.now() + CONFIG.GROQ_BACKOFF_MS; state.deliveryError = errorMessage; saveState_(file, state); return 'WAIT'; }
  return stageFailureForDelivery_(file, state, `CodeAgent stopped during ${stageName}: ${errorMessage}`);
}

function stageFailureForDelivery_(file, state, reason) { state.stage = 'FAILURE_PENDING'; state.failureReason = reason; state.nextAttemptAt = Date.now(); saveState_(file, state); return 'PROCESSED'; }
...
function collectSourceText_(msg) {
  let text = ''; let attachments;
  try { attachments = msg.getAttachments({ includeInlineImages: false, includeAttachments: true }); } catch (_) { attachments = msg.getAttachments(); }
  for (const att of attachments) {
    const name = att.getName() || 'attachment'; const type = (att.getContentType() || '').toLowerCase();
...
function splitIntoChunks_(text) {
  if (!text) return [''];
  const budget = getChunkCharBudget_(); if (text.length <= budget) return [text];
  const chunks = []; let remaining = text;
  const patterns = [/\n\s*\n/g, /<\/[a-zA-Z0-9:-]+>\s*\n/g, /}\s*\n/g, /;\s*\n/g, /\n/g];
  while (remaining.length > budget) {
    const window = remaining.substring(0, budget); let splitAt = -1;
    for (const pattern of patterns) {
      pattern.lastIndex = 0; let match; let lastMatchEnd = -1;
    ...
    chunks.push(remaining.substring(0, splitAt)); remaining = remaining.substring(splitAt);
  }
  if (remaining.length) chunks.push(remaining); return chunks;
}
...
function summarizeFilePurpose_(text) {
  ...
  return callGroq_({ model: CONFIG.GROQ_MODEL, max_completion_tokens: 300, messages: [
    { role: 'system', content: 'Summarize this source code architecture and purpose in 2-3 concise sentences. Focus on major components, data flow, important APIs, and naming conventions. Do not rewrite code.' },
    { role: 'user', content: preview }
  ] });
}

function estimateTokensV8_(text) { return Math.max(1, Math.ceil(String(text || '').length / CONFIG.CHARS_PER_TOKEN_ESTIMATE)); }
function getSafeCompletionBudgetV8_(promptText) {
  const promptTokens = estimateTokensV8_(promptText); const available = CONFIG.TPM_LIMIT - CONFIG.PROMPT_OVERHEAD_TOKENS - promptTokens;
  return Math.max(256, Math.min(3200, available));
}
...
function processChunk_({ instruction, fileSummary, chunk, chunkIndex, totalChunks }) {
  ...
  return callGroq_({ model: CONFIG.GROQ_MODEL, max_completion_tokens: getSafeCompletionBudgetV8_(prompt), messages: [
    { role: 'system', content: 'You are an expert full-stack web developer and code repair agent. Preserve existing functionality, make targeted changes, and never invent personal information. For standalone HTML, always finish the complete document.' },
...
function callGroq_(payload) {
  ...
  const code = response.getResponseCode(); let data;
  ...
  const rawError = data && data.error && data.error.message ? String(data.error.message) : ''; const errorMessage = rawError.toLowerCase();
...
function finalizeJob_(state) {
  ...
  if (!state.isChunked) {
    ...
    const repair = callGroq_({ model: CONFIG.GROQ_MODEL, max_completion_tokens: getSafeCompletionBudgetV8_(repairPrompt), messages: [
      { role: 'system', content: 'Repair incomplete HTML. Return only a complete runnable document. Do not invent personal facts or contact details.' },
  ...
  const mergeTokens = estimateTokensV8_(mergePrompt); const canMerge = mergeTokens + CONFIG.PROMPT_OVERHEAD_TOKENS + 900 <= CONFIG.TPM_LIMIT;
  if (canMerge) {
    const result = callGroq_({ model: CONFIG.GROQ_MODEL, max_completion_tokens: getSafeCompletionBudgetV8_(mergePrompt), messages: [
      { role: 'system', content: 'Merge transformed fragments into one complete runnable HTML document. Preserve functionality and do not invent personal information.' },
...
function makePreviewUrl_(htmlFile) {
  ...
  try { htmlFile.setSharing(DriveApp.Access.ANYONE_WITH_LINK, DriveApp.Permission.VIEW); }
  catch (err) { console.warn('Public Drive sharing unavailable; falling back to Drive URL.', err); }
...
function extractHtml_(text) {
  if (!text) return '';
  const htmlMatch = String(text).match(/```html\s*([\s\S]*?)```/i); if (htmlMatch) return htmlMatch[1].trim();
  const genericMatch = String(text).match(/```(?:[a-zA-Z0-9_-]+)?\s*([\s\S]*?)```/); if (genericMatch) return genericMatch[1].trim();
...
function sendFailureReply_(message, state, reason) {
  const parts = state.processedParts || []; const options = parts.length ? { attachments: [Utilities.newBlob(parts.join('\n\n'), MimeType.HTML, 'partial_recovery.html')] } : {};
  ...
  if (thread) {
    try {
      ...
      if (messages && messages.length) {
        const exact = state.messageId ? messages.find(m => m.getId() === state.messageId) : null; const message = exact || messages[messages.length - 1];
        if (message) { let recipient = ''; try { recipient = extractEmailAddress_(message.getFrom()); } catch (_) {} return { message, recipient }; }
      }
...
function extractEmailAddress_(fromField) { const text = String(fromField || '').trim(); const angle = text.match(/<([^>]+)>/); return (angle ? angle[1] : text).trim(); }
function getThreadSafely_(threadId) { try { return GmailApp.getThreadById(threadId); } catch (err) { if (isGmailQuotaError_(err)) pauseGmailAfterQuotaError_(err); else console.error('getThreadSafely_ failed:', err); return null; } }
function getMessageSafely_(thread, messageId) { try { const messages = thread.getMessages(); return messages.find(m => m.getId() === messageId) || messages[messages.length - 1] || null; } catch (err) { if (isGmailQuotaError_(err)) pauseGmailAfterQuotaError_(err); else console.error('getMessageSafely_ failed:', err); return null; } }
function markThreadReadSafely_(thread) { try { thread.markRead(); } catch (err) { if (isGmailQuotaError_(err)) pauseGmailAfterQuotaError_(err); else console.error('Could not mark thread read:', err); } }
function isGmailQuotaError_(err) { const message = String(err && err.message ? err.message : err).toLowerCase(); return /(service invoked too many times|too many times in a short time|daily limit|quota|rate limit)/i.test(message) && /(email|gmail|mail|recipient|send|message)/i.test(message); }
function gmailQuotaPauseActive_() { const value = getScriptProperties_().getProperty('GMAIL_QUOTA_PAUSED_UNTIL'); if (!value) return false; const timestamp = Number(value); if (!Number.isFinite(timestamp) || Date.now() >= timestamp) { getScriptProperties_().deleteProperty('GMAIL_QUOTA_PAUSED_UNTIL'); return false; } return true; }
function pauseGmailAfterQuotaError_(err) { const until = Date.now() + CONFIG.GMAIL_QUOTA_PAUSE_MS; const props = getScriptProperties_(); props.setProperty('GMAIL_QUOTA_PAUSED_UNTIL', String(until)); props.setProperty('LAST_GMAIL_ERROR', String(err)); }
function clearGmailPauseAndResume() { if (!isCodeAgentEnabled_()) throw new Error('CodeAgent is disabled. Run enableCodeAgent() first.'); const props = getScriptProperties_(); props.deleteProperty('GMAIL_QUOTA_PAUSED_UNTIL'); props.deleteProperty('LAST_GMAIL_ERROR'); console.log('Gmail local pause cleared. The stable worker will retry queued deliveries.'); }

function getOrCreateJobFolder_() { const folders = DriveApp.getFoldersByName(CONFIG.JOB_FOLDER_NAME); return folders.hasNext() ? folders.next() : DriveApp.createFolder(CONFIG.JOB_FOLDER_NAME); }
function saveState_(file, state) { state.updatedAt = new Date().toISOString(); file.setContent(JSON.stringify(state)); }
function createManagedTriggers_() { ScriptApp.newTrigger('processCodeEmails').timeBased().everyMinutes(CONFIG.POLL_MINUTES).create(); ScriptApp.newTrigger('resumeJobs').timeBased().everyMinutes(CONFIG.WORKER_MINUTES).create(); }
function ensureManagedTriggers_() { if (!isCodeAgentEnabled_()) return; const triggers = ScriptApp.getProjectTriggers(); const hasPoller = triggers.some(t => t.getHandlerFunction() === 'processCodeEmails'); const hasWorker = triggers.some(t => t.getHandlerFunction() === 'resumeJobs'); if (!hasPoller) { ScriptApp.newTrigger('processCodeEmails').timeBased().everyMinutes(CONFIG.POLL_MINUTES).create(); console.log('Self-healed missing processCodeEmails trigger.'); } if (!hasWorker) { ScriptApp.newTrigger('resumeJobs').timeBased().everyMinutes(CONFIG.WORKER_MINUTES).create(); console.log('Self-healed missing resumeJobs trigger.'); } }
...
function countQueuedJobs_() { try { const files = getOrCreateJobFolder_().getFilesByType(MimeType.PLAIN_TEXT); let count = 0; while (files.hasNext()) { files.next(); count++; } return count; } catch (err) { console.error('countQueuedJobs_ failed:', err); return -1; } }
...
function recoverStuckJobs() {
  ...
  ensureManagedTriggers_(); const files = getOrCreateJobFolder_().getFilesByType(MimeType.PLAIN_TEXT); let recovered = 0;
  while (files.hasNext()) { const file = files.next(); try { const state = JSON.parse(file.getBlob().getDataAsString()); if (state.stage === 'SEND_RESULT' || state.stage === 'FAILURE_PENDING') { state.nextAttemptAt = Date.now(); state.deliveryError = state.deliveryError || 'Manual recovery requested.'; saveState_(file, state); recovered++; } } catch (err) { console.error(`Could not inspect ${file.getName()}:`, err); } }
  console.log(`Recovered ${recovered} pending delivery job(s). Stable worker will handle them.`);
}

function inspectQueuedJobs() {
  const files = getOrCreateJobFolder_().getFilesByType(MimeType.PLAIN_TEXT); const jobs = [];
  while (files.hasNext()) {
    const file = files.next();
    try {
      const state = JSON.parse(file.getBlob().getDataAsString());
      jobs.push({ fileName: file.getName(), jobId: state.jobId || null, stage: state.stage || null, nextAttemptAt: state.nextAttemptAt || 0, nextAttemptIso: state.nextAttemptAt ? new Date(Number(state.nextAttemptAt)).toISOString() : null, currentIndex: Number(state.currentIndex || 0), totalChunks: Array.isArray(state.chunks) ? state.chunks.length : 0, finalizeRetryCount: Number(state.finalizeRetryCount || 0), deliveryAttempts: Number(state.deliveryAttempts || 0), recipientEmail: state.recipientEmail || null, deliveryError: state.deliveryError || null, failureReason: state.failureReason || null });
    } catch (err) { jobs.push({ fileName: file.getName(), error: String(err) }); }
  }
  console.log(JSON.stringify(jobs, null, 2)); return jobs;
}

/*
 * GitHub / Slack roadmap intentionally remains outside the core worker.
 * Future integrations should feed this same persistent queue rather than
 * creating separate AI pipelines.
 */
